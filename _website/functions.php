...
function footer_menu()
{
	$storage = Storage::instance();
	$out = '';
    $sql = "SELECT id, title, idx, redirectlink, menutitle, level, menuid, category FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND menuid = 5 AND deleted = 0 AND masterid = 0 AND visibility = 1 ORDER BY position asc;";
    $sections = db_fetch_all($sql);
    if (empty($sections))
        return NULL;
    for ($idx = 0, $out = NULL, $num = count($sections); $idx < $num; $idx++)
...
function location()
{
	...
	if($storage->section["id"]!=1)
		$out .= '<li class="item"><a href="' . href(c("section.home")) . '">' . $page["title"] . '</a></li>'."\n";
	$segment = '';
    if (is_numeric($storage->segments[count($storage->segments) - 1])) {
		if($storage->section["category"]==16) {
			for($i=0; $i<count($storage->segments)-2; $i++) :
				...
				$title = $page['title'];
				$out .= '<li class="item"><a href="' . $link . '">' . $title . '</a></li>'."\n";
			endfor;
			...
			for($i=0; $i<count($segments); $i++) :
				...
				$link = (($page['redirectlink'] == '') || ($page['redirectlink'] == 'NULL')) ? href($page['id']) : $page['redirectlink'];
				$title = $page['title'];
				$out .= '<li class="item"><a href="' . $link . '" class="active">' . $title . '</a></li>'."\n";
			endfor;
		...
		for($i=0; $i<count($storage->segments); $i++) :
			...
			$link = (($page['redirectlink'] == '') || ($page['redirectlink'] == 'NULL')) ? href($page['id']) : $page['redirectlink'];
			$title = $page['title'];
			$out .= '<li class="item"><a href="' . $link . '"'.(($i==count($storage->segments)-1) ? ' class="active"' : '').'>' . $title . '</a></li>'."\n";
		endfor;
	}
    return $out;
}
...
function main_menu2(){
	 $sql = "SELECT * FROM `pages` WHERE `language` = '" . l() . "' AND `menuid` = 1 AND `id` != 1 AND `deleted` = 0 AND `masterid` = 0 AND `visibility` = 1 ORDER BY `position` asc;";
    $fetch = db_fetch_all($sql);

    $out = "";
    
    foreach($fetch as $n){
    	$subSql = "SELECT * FROM `pages` WHERE `language` = '" . l() . "' AND `deleted` = 0 AND `masterid` = '".$n['id']."' AND `visibility` = 1 ORDER BY `position` asc;";
    	$subFetch = db_fetch_all($subSql);

    	if(isset($subFetch[0]['title'])){//has sub menu
    		$out .= "<li class=\"menu-dropdown-icon category-item nav-438 dropdown one-col-prod\">";
            $out .= sprintf("<a href=\"javascript:void(0)\" class=\"g-menuitem\"><span class=\"megaitemicons\"></span> %s</a>", $n['title']);
               
            $out .= "<span class=\"plus\"></span>";
            $out .= "<ul class=\"animated fadeIn column1\">";
            $out .= "<li class=\"megaStaticBlock\">";
            $out .= "<ul>";
			$out .= "<li>";
			$out .= "<ul class=\"animated fadeIn column2\" style=\"animation-duration: 0.3s;\">";
			$out .= "<li class=\"megaStaticBlock\">";
			$out .= "<ul>";
			$out .= "<li>";
			$out .= sprintf("<h4 class=\"pool-icon desktop-only g-menuitem\">%s</h4>", $n['title']);
			
			$out .= "<ul>";
			
			foreach($subFetch as $s):
			$out .= "<li>";
			$out .= "<span class=\"widget block block-category-link-inline\">";
			$out .= sprintf("<a href=\"%s\" class=\"g-menuitem-sub\"><span>%s</span></a>", href($s['id']), $s['title']);
			$out .= "</span>";
			$out .= "</li>";
			endforeach;
    		...
    		$out .= "<li>";
            $out .= sprintf("<a href=\"%s\" class=\"g-menuitem\">", href($n['id']));
            $out .= sprintf("<span class=\"megaitemicons\"></span> %s", $n['title']);
            $out .= "</a>";
        	$out .= "</li>";
    	}
    }
...
function g_categories($masterid = 0){
	$homepage = "";
	if($masterid<=0){
		$homepage = ' AND `homepage`=1';
	}
    $fecth = db_fetch_all("SELECT * FROM `pages` WHERE `language` = '" . l() . "' AND `menuid` = 1 AND `deleted` = 0 AND `masterid` = '".$masterid."' AND `visibility` = 1".$homepage." ORDER BY `position` asc;");
    return $fecth;
}

function g_strip_classes($html){
